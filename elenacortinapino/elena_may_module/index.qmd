...
```{r}
# Library loading and reading data
library(tidyverse)
...
getting_rid_of_up_to_form <- kungfu_strings |>
  # Placing characters up to the first space in column "Delete" for elimination
  separate_wider_regex(Form,
  patterns = c(
    delete = "^[^\\s]+",
    "\\s+",
    # Column "rest" stores the rest of the string for further edition
    rest = ".*"))
...
selecting_the_form <- getting_rid_of_up_to_form |>
  select(-delete) |>  
  # Every character in the string before "G" gets placed in column "Form"
  separate_wider_regex(rest,
  patterns = c(
    Form = ".*?(?=G)", 
    # Everything else in the string goes in column "Group"
    Group = ".*"))
```


#### Step 4 — Removing Defaulted Athletes
...
# Removing defaulted athletes
sample_kungfu <- kf|> filter(Rank != "Default") |>
  drop_na(Score) |> select(-X, -Rank, -Remark)
```
...
#### Step 5 — Standardizing Form Names
...
#### Step 7 — Missing Data Imputation Using KNN
...
kf10_imp_knn <- kNN(usable_sample_kungfu, k = 5) |>
  ...
  select(-`Group A_imp`,-`Group B_imp`,-`Group C_imp`,-`Group D_imp`,-`Group E_imp`,-`Group F_imp`, -recoded_forms_imp) 
...
```{r}
library(visdat)
vis_miss(usable_sample_kungfu|> select(-recoded_forms))
```
...
```{r}
write.csv(kf10_imp_knn, "matrix_kf_10.csv")
```
