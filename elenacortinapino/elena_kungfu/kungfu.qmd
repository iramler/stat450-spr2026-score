...
```{r}
hehe_kungfu <- sample_kungfu |> group_by(Group, Form) |> summarise(mean_score = mean(Score), .groups = "drop")|> pivot_wider(names_from = Form, values_from = mean_score)
```


```{r}
mat <- as.matrix(hehe_kungfu[,-1])
rownames(mat) <- hehe_kungfu$Group
```
