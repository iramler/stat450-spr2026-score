...
```{r}
kf_nodefault |> drop_na(Score) |> group_by(sex) |> summarise(mean(Score))
kf_nodefault |> drop_na(Score) |> group_by(Team) |> summarise(mean(Score))
kf_nodefault |> drop_na(Score) |> group_by(Form) |> summarise(mean(Score))
...
### Getting the data for other years
