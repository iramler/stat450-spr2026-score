...
year8goat <- read.csv("year8forsure.csv") |> drop_na(Score) |> mutate(Form_new = fct_recode(Form,
  ...
  "Other Traditional Single Weapon" = "Other Weapon")) |> group_by(Group, Form_new) |> summarise(mean_score = mean(Score), .groups = "drop")|> pivot_wider(names_from = Group, values_from = mean_score)
```
...
year8_imp_knn <- kNN(year8goat, k = 5) |> select(-A_imp,-B_imp,-C_imp,-D_imp,-E_imp,-F_imp, -`Form_new_imp`)
