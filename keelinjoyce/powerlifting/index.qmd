...
There are also additional exercises and extensions provided that can be assigned as work outside of class or a second class period.
:::
...
boxplot <- ggplot(data = female_bench_ranked, aes(x = AgeClass, y = Bench)) +
  geom_boxplot(fill = "#d55353") +
  labs(x = "Age Class", y = "Bench Weight", 
       title = "Boxplot of Bench Weight by Age Class") +
  theme_minimal()
...
logplot <- ggplot(dunn_results_log, aes(x = group1, y = group2, fill = log_p)) +
  geom_tile(colour = "black", linewidth = 0.5) +
  scale_fill_gradient(low = "#CD3333", high = "white") +
  labs(title = "-log10 p-values of Dunn's Test Results by Age Class", x = "Age Class", y = "Age Class", fill = "-log10 p-values") +
  theme_minimal()
...
-   A sample module for female squatting can be found here [Sample Module](Sample Powerlifting Module.html){target="_blank"}
:::
...
::: {.callout-note collapse="false" title="References and Additional Readings"}

-   [Understanding the Use of the Kruskal-Wallis Test](https://pubmed.ncbi.nlm.nih.gov/9413454/
) by the National Library of Medicine

-   [Background on How to run Dunn's Test](https://rpkgs.datanovia.com/rstatix/reference/dunn_test.html
){target="_blank"} from rstatix

-   [Statology Info on Dunn's Test](https://www.statology.org/dunns-test/){target="_blank"} by Zach Bobbitt
