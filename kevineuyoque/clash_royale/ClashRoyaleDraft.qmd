...
- [tidyverse](https://tidyverse.org/packages/){target="_blank"}
- [igraph](https://r.igraph.org/){target="_blank"}
- [vembedr](https://www.rdocumentation.org/packages/vembedr/versions/0.1.5){target="_blank"}
- [netrankr](https://schochastics.github.io/netrankr/){target="_blank"}
...
# speak about the process on creating the network

# make a table for the cards in the network with their centrality

# go into depth with the meta and explain why centrality is important with these certain cards
...
pair_data_16 <- pair_data |>
  ...
  mutate(synergy = ifelse(winrate > 0.58, 1, 0)) |>
  #filter(synergy == 1) |>
...
```
# rather than change code, explain why some networks might not have all 16 cards included, meaning those cards would be excluded since they do not work well together
