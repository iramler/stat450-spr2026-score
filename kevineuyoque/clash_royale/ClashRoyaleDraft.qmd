...
I created a dataset containing all 125 cards, and the next step is to transform it into a card by card matrix that summarizes pair frequency, wins, and win rates.
...
We will focus on Season 3 deck data.
...
# Pick 16 cards from the dataset for which we will use

# go back to adjacency matrix 
# only 16 nodes and only have edges for nodes with higher than 50% win rate
...
plot(network_16,
     ...
     vertex.size = 8,
     vertex.label.cex = .8,
     edge.width = E(network_16)$winrate * 4)
