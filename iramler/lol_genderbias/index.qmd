...
To begin, we will need to load the packages we will be using. For this analysis, you'll need to load the `dplyr`, `tidyr`, `vcd`, and `viridis` packages.
...
# install.packages(c("dplyr,"tidyr", "vcd", "ggplot2", "viridis"))
...
These values (the totals, not the percents), are called *expected counts* in a Chi-square test. In general, they can be calculated using any of the three equivalent forms below.
...
The **chi-square test statistic** measures the amount that our expected counts differ from our observed counts. More specifically, it is calculated by using:

$$
\chi^2 = \sum_{cell}\frac{(O-E)^2}{E}
$$
where 

- $O$ is the observed count in the cell
- $E$ is the expected count in the cell
- $\sum$ means to add the result from each cell

Intuitively, this statistic works well as the ($O-E$) portion provides the difference between what we observe and what we expect under the null hypothesis. We square this difference to ensure we always get a positive value. We divide by the expected count to control for the size of the cell. In terms of our application, we can think of it as accounting for the fact that we have fewer female champions than male regardless of which lane they are in.
...
> Question: By hand, calculate the contribution to the test statistic associated with a female champion being a support.
...
```{r}
numerator <- (lol_2way_table_raw - lol_expected_counts)^2

test_stat <- sum( numerator/lol_expected_counts )
test_stat
```
```{r}
chisq.test(lol_2way_table_raw, simulate.p.value = TRUE, B = 5000)
```
