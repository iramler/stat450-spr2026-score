...
To begin, we will need to load the packages we will be using. For this analysis, you'll need to load the `dplyr`, `tidyr`, `vcd`, `viridis`, and `visualize` packages.
...
# install.packages(c("dplyr,"tidyr", "vcd", "ggplot2", "viridis", "visualize"))
...
We can also visualize this $p$-value using the `visualize.chisq()` function from the `visualize` package:

```{r}
vdist_chisquare_prob(perc = 13,
  df = 11,
  type = c("lower", "upper"),
  print_plot = TRUE)


visualize.chisq(stat = test_stat, df = dof, 
                type = "upper")
