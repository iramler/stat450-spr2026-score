...
```{r}
lol_2way_table <- table(lol_MFonly$gender, lol_MFonly$lane)
```
...
```{r}
signif(prop.table(x = lol_2way_table, margin = 2), 2)
```
...
For example, we can ask:
  - *Within each lane, what proportion of champions are male vs. female (and other/unknown)?*  
  - *Within each gender, what proportion of champions are assigned to each lane?*  
  These conditional distributions are often the most interpretable way to compare categories.

- **Bar charts:** A side-by-side (grouped) bar chart displays the counts by lane with separate bars for gender. A 100% stacked bar chart displays proportions, which makes it easier to compare the **gender mix** across lanes.

- **Mosaic plot:** A mosaic plot visualizes the contingency table directly. Each rectangle’s **area** is proportional to the count in that cell, and the overall pattern provides a compact view of whether the variables look independent (similar gender proportions across lanes) or associated (noticeably different proportions across lanes).

In the next section, we will create each of these summaries in R to look for evidence of association before running a formal hypothesis test.
...
- Two-ways tables: Also known as a *contingency table*...
