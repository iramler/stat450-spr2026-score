...
ggplot(df_diff, aes(x = donuts, y = time_sec_adjusted)) +
...
ggplot(df_diff2, aes(x = donuts, y = time_sec_adjusted)) +
  geom_point(fill = "#d95f02") +
  geom_smooth() +
  labs(
...
```{r}
quad_model <- lm(time_sec_adjusted ~ donuts + I(donuts^2), data = df_diff2)
summary(quad_model)

b <- coef(quad_model)
opt_donuts <- -b["donuts"] / (2 * b["I(donuts^2)"])
opt_donuts
