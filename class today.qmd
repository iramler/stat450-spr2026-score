...
X5KRun1 <- read_csv("~/Downloads/5KRun1.csv") %>%
  mutate(Time = as.character(Time))|> select(1:4) # Ensure this is text

X5KRunUnadj1 <- read_csv("~/Downloads/5KRunUnadj1.csv") |> select(1:4)
...
library(ggplot2)
df_diff
df_diff <- df_diff |> filter(donuts >= 1) |> mutate(
  position = case_when(
      Position <= 10  ~ "Top 10",
      Position <= 25  ~ "Top 25",
      Position <= 50 ~ "Top 50",
      Position <= 100 ~ "Top 100",
      Position <= 150 ~ "Top 150",
      Position <= 200 ~ "Top 200",
      Position >= 200 ~ "Top 258"
...
```{r}
df_differ <- df_diff |> mutate(timed = time_sec_adj / 60) |> mutate(timer = time_sec_unadj / 60)
df_me <- df_differ |> select(Position, timer, donuts, timed) |> arrange(timed)
df_me
df_differ
ggplot(data = df_differ, aes(x = donuts, y = timed, colour = Position))+
  geom_jitter()
...
```{r}
ggplot(df_differ, aes(x = timed, y = timer)) +
  ...
  theme_minimal()
library(broom)
mod_don = lm(timed ~ timer + donuts, data = df_differ)
summary(mod_don)
mod_don
residi_dono = augment(mod_don, df_differ)
residi_dono |> summarise(mean = mean(.hat))

residi_dono |> summarise(mean = mean(abs(.std.resid)))
...
0.0338-0.0507
3*0.0169
residi_dono |> arrange(desc(abs(.std.resid))) |> slice(1:19) |> select(timed, timer, .std.resid, .hat, Position, donuts)
residi_dono |> arrange(timed) |> slice(1:19) |> select(timed, timer, .std.resid, .hat, Position)

residi_dono |> filter(donuts >=9) |> select(timed, timer, .std.resid, .hat, Position, donuts) |> arrange(desc(donuts))|> print(n = 26)
plot(mod_don)
```
